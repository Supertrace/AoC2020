...
type Board = M.HashMap (Int,Int,Int,Int) Char
...
mapConst ((row, y):rest) mep = mapConst rest (M.union mep newMep) where
  rowList = map (\(x,c) -> ((x,y,0,0), c)) row
  newMep  = M.fromList rowList

surrounding :: (Int, Int, Int,Int) -> Board -> String
surrounding loc@(x,y,z,w) board = unMaybe $ map (`M.lookup` board) otherLocs where
  otherLocs' = [(x',y',z',w') | x' <- [x-1..x+1], y' <- [y-1..y+1], z' <- [z-1..z+1], w' <- [w-1..w+1]]
...
update :: Board -> (Int, Int, Int, Int) -> ((Int,Int,Int,Int), Char)
...
oneStep (x, y, steps, board) = (x, y, steps + 1, newBoard) where
  lowerB   = -steps
  range    = [(x,y,z,w) | x <- [lowerB..(x + steps)], y <- [lowerB..(y + steps)], z <- [lowerB..(-lowerB)], w <- [lowerB..(-lowerB)]]
